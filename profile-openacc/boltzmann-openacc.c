...
int main(int argc, char* argv[]) {
    ...
    printf("d2q9-bgk lattice Boltzmann scheme: optimised openacc code\n");
...
int collision(const t_param params, t_speed* src_cells, t_speed* dst_cells, unsigned char* obstacles) {
     ...
     */
#pragma acc parallel loop collapse(2) private(ii, jj, x_w, x_e, y_s, y_n, speeds, d_equ, local_density, u_x, u_y, u_sq, u) copyin(params, src_cells, obstacles) copyout(dst_cells)
    for (ii = 0; ii < params.ny; ii++) {
        for (jj = 0; jj < params.nx; jj++) {
            int offset = ii * params.nx + jj;
            int x_w = params.lookup_x_w[jj];
            int x_e = params.lookup_x_e[jj];
            int y_s = params.lookup_y_s[ii];
            int y_n = params.lookup_y_n[ii];

            double *dst_speeds = dst_cells[offset].speeds;
            double *src_speeds = src_cells[offset].speeds;

            if (obstacles[offset]) {
                dst_speeds[0] = src_speeds[0];
                ...
                u[1] = u_x; u[2] = u_y; u[5] = u_x + u_y; u[6] = -u_x + u_y;
                ...
                memcpy(dst_speeds, speeds, sizeof(speeds));
#endif
            }
        }
    }
...
#pragma acc parallel loop reduction(+:tot_u_x,tot_cells)
    for (ii = 0; ii < params.ny; ii++) {
#pragma acc loop
        for (jj = 0; jj < params.nx; jj++) {
...
#pragma acc parallel loop collapse(3) reduction(+:total)
    for (ii = 0; ii < params.ny; ii++) {
        for (jj = 0; jj < params.nx; jj++) {
            for (kk = 0; kk < NSPEEDS; kk++) {
...
#pragma acc update host(u_x, u_y, pressure)
    for (ii = 0; ii < params.ny; ii++) {
        for (jj = 0; jj < params.nx; jj++) {
            int offset = ii * params.nx + jj;

...
int main(int argc, char* argv[]) {
    ...
    #pragma acc parallel
    {
        #pragma acc host_data use_device(num_threads)
        {
            int num_threads;
            #pragma acc parallel
            {
                #pragma acc atomic read
                num_threads = acc_get_num_threads();
            }
...
int accelerate_flow(const t_param params, t_speed * cells, unsigned char* obstacles) {
    ...
    #pragma acc parallel loop private(speeds, ii, offset) \
        copyin(cells, params, obstacles, w1, w2)
...
int collision(const t_param params, t_speed* src_cells, t_speed* dst_cells, unsigned char* obstacles) {
     ...
     */
     #pragma acc parallel loop private(ii, jj, kk, offset, speeds, dst_speeds, local_density, u_x, u_y, u_sq, u, d_equ, x_e, x_w, y_n, y_s) \
        copyin(dst_cells, src_cells, params, obstacles, w0, w1, w2)
    for (ii = 0; ii < params.ny; ii++) {
...
double av_velocity(const t_param params, t_speed* cells, unsigned char* obstacles) {
    ...
    #pragma acc parallel for reduction(+: tot_cells, tot_u_x) \
        copyin(obstacles, cells) \
        private(ii, jj, offset, speeds, local_density)
    for (ii = 0; ii < params.ny; ii++) {
        for (jj = 0; jj < params.nx; jj++) {

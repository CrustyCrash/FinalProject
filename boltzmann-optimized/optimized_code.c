...
#define NUM_SPEEDS         9
#define PARAMETER_FILE       "input.params"
#define OBSTACLE_FILE    "obstacles_300x200.dat"
#define FINAL_STATE_FILE  "final_state%s%s.dat"
#define AVERAGE_VELOCITY_FILE      "av_vels%s%s.dat"
...
int main(int argc, char* argv[]) {
    ...
    printf("d2q9-bgk lattice Boltzmann scheme: optimised serial code\n");
...
#endif
    if (argc > 1) {
        sprintf(finalStateFile, FINAL_STATE_FILE, ".", argv[1]);
        sprintf(avVelocityFile, AVERAGE_VELOCITY_FILE, ".", argv[1]);
    } else {
        sprintf(finalStateFile, FINAL_STATE_FILE, "", "");
        sprintf(avVelocityFile, AVERAGE_VELOCITY_FILE, "", "");
    }
...
int collision(const t_param params, t_speed* src_cells, t_speed* dst_cells, unsigned char* obstacles) {
    ...
    double u[NUM_SPEEDS];              /* directional velocities */   
    double d_equ[NUM_SPEEDS];          /* equilibrium densities */   
